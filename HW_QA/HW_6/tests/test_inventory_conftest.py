...
class TestInventory:
    ...
    def test_Bolt_T_Shirt_cost(self):
        self.login_page.open()
        self.login_page.success_login("standard_user", "secret_sauce")

        price = self.inventory_page.get_item_price("Sauce Labs Bolt T-Shirt")
        self.inventory_page.add_item_to_cart("Sauce Labs Bolt T-Shirt")
        self.inventory_page.go_to_cart()

        cart_price = self.cart_page.get_cart_item_price("Sauce Labs Bolt T-Shirt")
        assert price == cart_price
